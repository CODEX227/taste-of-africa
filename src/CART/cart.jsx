import { FiShoppingCart, FiMap, FiCreditCard, FiCheck, FiTrash, FiTrash2, FiTruck, FiHeadphones, FiLock, FiEdit2, FiEdit, FiEdit3, FiDollarSign, FiArrowRight, FiLoader } from "react-icons/fi";
import { IoLocationOutline } from "react-icons/io5";
import { AddAppends, addorder, clearcart, dec, inc, OngoingPends, removecart } from "../../DATABASE/handleUser";
               ...
               try {
                    const response = await fetch(
                        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/flutterwave`,
                        {
                            ...
                            body: JSON.stringify({
                                amount: information.cart && information.cart[info.uid] ? 750 + Object.values(information.cart[info.uid]).reduce((sum, each) => {return sum + information.food.foodlisting[each.foodid].price * each.quantity} ,0) + Math.round(1 / 100 * Object.values(information.cart[info.uid]).reduce((sum, each) => {return sum + information.food.foodlisting[each.foodid].price * each.quantity} ,0)) : '',
                                email: information.users[info.uid].email,
                                phone: information.users[info.uid].delivery.phone,
                                name: information.users[info.uid].name,
    ...
    return ( 
        <div className="cart-page">
            ...
            {editable.status ? 
                    <div className="editable-con" onClick={(e) => {if(e.target.className == 'editable-con'){seteditable({})}}}>
                        ...
                        {editable && <div className="editable-food-con">
                            {
                               Object.values(information.food.foodlisting[editable.foodid].categories).map((each, index) => {
                                return (
                                    <div className="go-for">
                                            ...
                                            each.type == 'main' ?
                                             <select  onChange={(e) => {AddAppends(info.uid, editable.cartid, {selected : e.target.value}, each.name)}}>
                                                {Object.values(each.exchange.to).map((eachOp, index) => {
                                                    return(
                                                         <option className="opt">{eachOp}</option>
                                                    )
                                                })}
                                             </select> 
                                            : each.type == 'edit' ? 
            ...
            <div className="order-status-con">
                <div className="status-con-each"><FiShoppingCart className={information.cart &&  information.cart[info.uid] ? 'status-i' : 'status-i not-ready'} size = {12} />Cart</div>
                <div className= {information.cart &&  information.cart[info.uid] ? 'line' : 'line not-ready'}></div>
                ...
                <div className="status-con-each"><FiCheck className="status-i not-ready" style={{backgroundColor : 'green'}}  size={12} />Pay</div>
            </div>
            <div className="others-con">
                ...
                <div className="whole-det-con">
                    ...
                    {
                      information.cart &&  information.cart[info.uid] ? (

                            <div>
                                { Object.values(information.cart[info.uid]).map((each, index) => {
                       return (
                       <>
                ...
                <div className="all-sums">
                            ...
                            <div className="sum-det">
                                <span className="each-sum-det">Subtotal({Object.values(information.cart[info.uid] || {}).length} item) <span className="inner-sum">₦{information.cart ? Object.values(information.cart[info.uid] || {}).reduce((sum, each) => {return sum + information.food.foodlisting[each.foodid].price * each.quantity} ,0) : '0'}</span></span>
                                <span className="each-sum-det">Delivery Fee <span className="inner-sum">₦750</span></span>
                                <span className="each-sum-det">Service Charge(1%) <span className="inner-sum">₦{information.cart ? Math.round(1 / 100 * Object.values(information.cart[info.uid] || {}).reduce((sum, each) => {return sum + information.food.foodlisting[each.foodid].price * each.quantity} ,0)) : '0'}</span></span>
                                <span className="each-sum-det-dif">Total <span className="inner-sum">₦{750 + Object.values(information.cart[info.uid] || {}).reduce((sum, each) => {return sum + information.food.foodlisting[each.foodid].price * each.quantity} ,0) + Math.round(1 / 100 * Object.values(information.cart[info.uid] || {}).reduce((sum, each) => {return sum + information.food.foodlisting[each.foodid].price * each.quantity} ,0))}</span></span>
                            </div>
                            <div className="free-delivery">
                                <div className="frre-d-head"><MdDeliveryDining size={28} className="free-i"/><span className="free-sec-con"> You are now <span className="free-digit">34%</span> away fom <span className="free-digit">FREE DELIVERY!!</span></span></div>
                                <div className="count"><div className="inner-count"></div></div>
                                <div className="free-del-btn"><button>Use Now</button></div>
                            </div>
                        </div>}
                        ...
                        <div className="summary">
                            ...
                            <div className="sum-det">
                                <span className="cart-trust">
                                 <FiTruck size = {20} className="card-trust-i"/> 
                                ...
                                <span className="cart-trust">
                                 <MdFactCheck size = {20} className="card-trust-i"/> 
                                  <div className="inner-cart-trust">
                                ...
                                <span className="cart-trust">
                                 <FiLock size = {20} className="card-trust-i"/> 
                                  <div className="inner-cart-trust">
                                ...
                                <span className="cart-trust">
                                 <FiHeadphones size = {20} className="card-trust-i"/> 
                                  <div className="inner-cart-trust">
                        ...
                        <div className="summary" style={{ backgroundColor: 'rgb(0, 0, 0, .18)', boxShadow : 'none', paddingBottom :'20px'}}>
                            <span className="sum-head" style={{color: 'black'}}>Have a promo code?</span>
                ...
                <div className="del-info-con">
                    <span className="del-info-head"><IoLocationOutline size = {20} />Delivery Info</span>
                    <span className="del-info-2">Delevery Address</span>
                    <span className="del-info-info">{!information.users[info.uid].delivery ? 'Not recognized' : information.users[info.uid].delivery  ? information.users[info.uid].delivery.adress : null}</span>
                    <button className="del-info-btn">Edit</button>
                </div>
                <div className="del-info-con">
                    <span className="del-info-head"><FiTruck size = {20} />Delivery Option</span>
                    <span className="del-info-2 info-2-diff"><input type="radio" value="delivery" checked = {DelMethod === "delivery"} onChange={(e) => {setDelMethod(e.target.value)}}/>Standard Delivery</span>
                    <span className="del-info-info info-2-diff"><input type="radio" value="pickup" checked = {DelMethod === "pickup"} onChange={(e) => {setDelMethod(e.target.value)}}/>Pickup</span>
                </div>
                <div className="del-info-con">
                    <span className="del-info-head"><FiCreditCard size = {20} />Delevery Fee</span>
            ...
            <div className="checkout">
                ...
                <button onClick={() => {
                    if(!information.cart[info.uid]){
                        console.log('no food')
                        setfooderr(!fooderr)
                        return
                    }else if(!information.users[info.uid].delivery){
                        setshowerr(!showerr)
                        return
                    }
                    else if(information.users[info.uid].delivery && information.cart){
                        const orderid = Math.random().toString(36).slice(2, 12)
                        OngoingPends(info.uid, orderid)
                        addorder(info.uid, {
                            status:'pending',
                            transaction_id:null,
                            items:information.cart[info.uid],
                            total:null,
                            location:information.users[info.uid].delivery.adress
                        }, orderid)
                        setloadingpaymentmethod(true)
                        Payshit()
                    }
                }}>{loadingpaymentmethod ? <FiLoader size={17} style={{animation : 'load 0.1s ease-in-out infinite '}}/> : 'Proceed to Checkout'} <FiArrowRight size = {14} /></button>
            </div>
